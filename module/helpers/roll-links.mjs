 ...
 */
const CUE_FILLER = "(?:a|an|the|another|one|two|three|four|five|six|\\d+)";
...
const INSTRUCTION_CUE =
  `(?<=\\b(?:makes?|making|rolls?|rolling|requires?|attempts?)\\s+(?:${CUE_FILLER}\\s+)*)`;
 ...
 *   - any phrase with no instruction cue in front of it, per INSTRUCTION_CUE
 */
